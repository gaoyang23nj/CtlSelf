...
excluding $src$, namely $N=100$.
The Poisson-contact mobility model is synthetic,
%quasi-synthetic,
%in which
where the parameter $\lambda$ is set as
$4 \times 10^{-3}$.
...
We set the parameter $\rho$ as $1.1 \times 10^{-2}$.
The weight $\alpha$, $\alpha \in (0, 1)$, is a constant factor.
The minimum detection cycle is that $T_{m}=1$s.
Note that all numerical results in the experiments
...
and further achieve the optimal control
%based on the optimal control achieved
...
around $102s$ and $452s$,
dominate the cost $J$ in the whole simulation.
Combining Fig.~\ref{fig:pe_opt_state_time} and
Fig.~\ref{fig:pe_opt_control_Ut},
we can also find that neither $D(t)$ nor $\lambda_{D}(t)$
is smooth at the switching time.
...
\begin{figure}
  \centering
  {\includegraphics[width=0.45\textwidth]{fig/cost_all_t0t1.eps}}
     \caption{Total cost $J$ vs. ($t_{on}$, $t_{off}$).}
     \label{fig:cost_ton_toff}
\end{figure}
\begin{figure}
  \centering
  {\includegraphics[width=0.45\textwidth]{fig/reward_all_t0t1.eps}}
     \caption{Total reward $P$ vs. ($t_{on}$, $t_{off}$).}
...
when $\alpha=0.9$ and $T=500s$.
Here the combination $(t_{on}, t_{off})$ spans
from $(0, 0)$ to $(500, 500)$
...
in our experiments.
Additionally, the time granularity of $0.1s$
during $20$ times simulations.
We can find that $J$ ranges from $1,100$ to $1,700$.
The optimal solution according to our method is that
$t_{on}=102.1s$ and $t_{off}=402.59s$,
...
where the unit reward $\beta$ in (\ref{eq:reward}) is set as $0.1$.
We can find that the minimum reward around $1,900$
at ($0$,$500$),
...
Meanwhile, the utilization ratio of reward $p$
...
the detected nodes.
The method with complete detection outperforms
the method without detection in terms of the cost $J$
...
where the detection cycle in the message lifetime
is $2T_{m} = 2s$,
is between the the method with complete detection
