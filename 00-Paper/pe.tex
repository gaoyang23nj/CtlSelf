...
the detection is off at the start 
...
Thus the switching time $t_{s}$ and $t_{t}$,
...
is not smooth at the switching time.

[*** new line ***].
When $\alpha=$ is higher than $\alpha_{th}$
the optimal control policy $U^{*}(t) = 0$, $\forall t$,
