...
\STATE {conduct the detection each $T_{m}$ from $\tau_{0}$ to $\tau_{1}$}
\WHILE {$t$}
    \IF {contact $n_{i}$ without message}
        \STATE {replicate $m$ to $n_{i}$}
        \STATE {record $t$ as $\tau_{0}$ in the table}
    \ENDIF
    \IF {$t$ $\in$ $\mathcal{T}$}
        \STATE {conduct the detection about the randomly selected node $n_{j}$}
        \STATE {change the node state of $n_{j}$}
    \ENDIF
\ENDWHILE
\STATE {pay the reward according to the contribution time of each node}
\end{algorithmic}
\end{small}
\end{algorithm}
