...
Thus $I(t)$ in this section is the same as
$I(t)$ in the case without detection, which can be computed as
\begin{equation}
\begin{small}
%\nonumber
\label{eq:I}
...
\nonumber
\begin{aligned}
J &= \int_{0}^{T} \big( (1-\alpha) D(t) + \alpha U(t) \big) dt.
\end{aligned}
\end{small}
\end{equation}
...
\WHILE {$t \le T$}
    \IF {contact $n_{i}$ without message}
        ...
        \STATE {$src$ records $t$ as the message replication time}
    \ENDIF
    \IF {$t$ $\in$ $\mathcal{T}$}
        \STATE {select a relay node $n_{j}$ randomly}
        \STATE {$src$ conducts the selfish node detection to $n_{j}$}
            \IF {$n_{j}$ is detected as a selfish node}
                \STATE {state of $n_{j}$ changes to state $R$}
                \STATE {$src$ records $t$ as the state switching time}
            \ENDIF
    \ENDIF
    \STATE {$t \leftarrow t + \delta t$}
\ENDWHILE
\FORALL {$i \in [1, N]$}
    \STATE {pay reward to $n_{i}$ based on the message carrying time}
\ENDFOR
\end{algorithmic}
\end{small}
\end{algorithm}
...
At the beginning and the end of the message lifetime,
the optimal control of the selfish node detection is off,
which means that $U^{*}(0)=U^{*}(T)=0$.
\end{lem}

\begin{proof}
At the beginning of the duration, $D(0)=0$,
which is the initial condition of (\ref{eq:bvp}).
Then $\alpha - \lambda_{D}(0) \frac{D(0)}{N} = \alpha > 0$.
Thus the optimal $U^{*}(0)=0$ according to (\ref{eq:opt_U}), .

Similarly, $\lambda_{D}(T)=0$,
which is the boundary condition of (\ref{eq:bvp}).
Then we have $\alpha - \lambda_{D}(T) \frac{D(T)}{N} = \alpha > 0$.
...
If $\alpha \ge \alpha_{th}$, 
the optimal control of the detection is off
within the message lifetime,
namely $U(t)=0$, $\forall t \in [0, T]$.
\end{lem}
...
- (1-\alpha)D(t^{*})=0$.
According to $W^{'}(t^{*}) = 0$,
$D(t^{*}) = \frac{\rho \lambda_{D}(t^{*}) I(t^{*})}{1-\alpha}$.
\begin{equation}
\begin{small}
%\nonumber
\label{eq:W_t_star}
