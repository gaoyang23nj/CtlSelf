...
The message replication time $\tau_{0}$ and the state switching time are recorded by $src$.
...
\WHILE {$t \le T$}
    \IF {contact $n_{i}$ without message}
        ...
        \STATE {state of $n_{i}$ changes to $I$}
        \STATE {record $t$ as $\tau_{0}$}
    \ENDIF
    \IF {$t$ $\in$ $\mathcal{T}$}
        \STATE {select a relay node $n_{j}$ randomly}
        \STATE {conduct the detection of $n_{j}$}
            \IF {$n_{j}$ is detected as a selfish node}
                \STATE {state of $n_{j}$ changes to $R$}
                \STATE {record $t$ as the state switch time}
            \ENDIF
    \ENDIF
    \STATE {$t \leftarrow t + \delta t$}
\ENDWHILE
\FOR {$n_{i}$, $1 \le i \le N$}
    \STATE {pay the reward to $n_{i}$ based on the time of staying state $I$}
\ENDFOR
\end{algorithmic}
\end{small}
\end{algorithm}
...
=& \left(\rho I(t)  - \beta D(t) 
- \frac{D(t)}{N} U(t) \right)\lambda_{D}(t) \\
&+ D(t) \left(\lambda_{D}(t) \left( \beta + \frac{U(t)}{N} \right) 
- (1-\alpha) \right)\\
...
is that $\lambda_{D}^{*} = \frac{1-\alpha}{\beta+\frac{U}{N}}$.
Since $0 \le U \le U_{m}$, $0 < \frac{1-\alpha}{\beta+\frac{U_{m}}{N}} \le \lambda_{D}^{*} \le \frac{1-\alpha}{\beta}$.
Note $\lambda_{D}(T)=0$.
...
Thus $0 \le \lambda_{D}(t) \le \lambda_{D}^{*} \le \frac{1-\alpha}{\beta}$ when $0 \le t \le T$.
...
which means that $\alpha \ge \frac{\rho}{\beta(\beta+\rho)+\rho} 
= \alpha_{th}$.
